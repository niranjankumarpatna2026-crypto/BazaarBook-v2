...
export function RecentBills({ bills, onOpen }: Props) {
  return (
    <div className="card overflow-hidden">
      <div className="flex items-center justify-between border-b border-stone-100 px-5 py-4">
        <h3 className="font-display text-base font-bold text-slate-900">
        ...
        <ul className="divide-y divide-stone-100">
          {bills.slice(0, 5).map((b) => (
            <li key={b.id}>
              <button
                onClick={() => onOpen?.(b.id)}
                className="flex w-full items-center gap-3 px-5 py-3 text-left transition hover:bg-stone-50 active:bg-stone-100"
              >
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-stone-100 to-stone-50 text-sm font-bold text-slate-700">
                  {initial(b.customerName)}
                </span>
